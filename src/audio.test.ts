...
function onRun(
  handler: (args: string[]) => { stdout?: string; stderr?: string; error?: Error },
) {
  execFileMock.mockImplementation(
    (_file: string, args: string[], cb: (e: Error | null, out: string, err: string) => void) => {
...
describe("getDevices", () => {
  it("returns input devices with isCurrent flag set from the default", async () => {
    ...
    expect(execFileMock).toHaveBeenCalledWith(BINARY, ["list", "--input", "--json"], expect.any(Function));
    expect(execFileMock).toHaveBeenCalledWith(BINARY, ["input", "get", "--json"], expect.any(Function));
  });
  ...
  it("uses output CLI args for output type", async () => {
    ...
    expect(execFileMock).toHaveBeenCalledWith(BINARY, ["list", "--output", "--json"], expect.any(Function));
    expect(execFileMock).toHaveBeenCalledWith(BINARY, ["output", "get", "--json"], expect.any(Function));
    // never touched the input path
    expect(execFileMock).not.toHaveBeenCalledWith(BINARY, ["list", "--input", "--json"], expect.any(Function));
  });
  ...
  it("a dual-role device appears in both lists with independent isCurrent", async () => {
    ...
    onRun((args) => {
      ...
      if (args[0] === "input") return { stdout: JSON.stringify({ id: 7, name: "USB Interface" }) };
...
describe("getCurrent", () => {
  ...
  it("uses the output CLI path for output type", async () => {
    onRun(() => ({ stdout: JSON.stringify({ id: 5, name: "Studio Display" }) }));
    const current = await getCurrent("output");
    expect(current).toEqual({ id: 5, name: "Studio Display", isCurrent: true });
    expect(execFileMock).toHaveBeenCalledWith(BINARY, ["output", "get", "--json"], expect.any(Function));
    expect(execFileMock).not.toHaveBeenCalledWith(BINARY, ["input", "get", "--json"], expect.any(Function));
  });
...
describe("setDevice", () => {
  it("calls `input set <id>` for input", async () => {
    ...
    expect(execFileMock).toHaveBeenCalledWith(BINARY, ["input", "set", "2"], expect.any(Function));
  });

  it("calls `output set <id>` for output", async () => {
    onRun(() => ({}));
    await setDevice("output", 5);
    expect(execFileMock).toHaveBeenCalledWith(BINARY, ["output", "set", "5"], expect.any(Function));
  });
